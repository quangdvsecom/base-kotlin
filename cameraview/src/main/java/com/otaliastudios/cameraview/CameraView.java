...
public class CameraView extends FrameLayout implements LifecycleObserver {
    ...
    final static boolean DEFAULT_USE_DEVICE_ORIENTATION = false;
...
//                super.onMeasure(w, h);
                int widthSpec = MeasureSpec.makeMeasureSpec(w, EXACTLY);
                int heightSpec = MeasureSpec.makeMeasureSpec(h, EXACTLY);
                super.onMeasure(widthSpec, heightSpec);
                return;
            }
            ...
            if (widthMode == EXACTLY && heightMode == EXACTLY) {
                Log.d("onMeasure:", "both are MATCH_PARENT or fixed value. We adapt. This means CROP_CENTER. (" + widthMeasureSpec + "x" + heightMeasureSpec + ")");
                int w = Math.max(widthMeasureSpec, 1);
                int h = Math.max(heightMeasureSpec, 1);
//                super.onMeasure(w, h);
                int widthSpec = MeasureSpec.makeMeasureSpec(w, EXACTLY);
...
//                        MeasureSpec.makeMeasureSpec((int) previewHeight, EXACTLY));
                int widthSpec = MeasureSpec.makeMeasureSpec((int) previewWidth, EXACTLY);
                int heightSpec = MeasureSpec.makeMeasureSpec((int) previewHeight, EXACTLY);
                super.onMeasure(widthSpec, heightSpec);
                return;
            }
